...
export class Movement {
  ...
  constructor(movemmentDto?: MovemmentDto) {
    this.accIdIncome = movemmentDto?.accIdIncome ?? '';
    this.accIdOutcome = movemmentDto?.accIdOutcome ?? '';
    this.movReason = movemmentDto?.movReason ?? '';
    this.movAmount = movemmentDto?.movAmount ?? '';
    this.movFees = movemmentDto?.movFees ?? 0;
  }
}
